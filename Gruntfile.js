...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    bump: {
      options: {
        ...
        updateConfigs: ['pkg'],
        commit: true,
        commitMessage: 'Build v%VERSION% --skip-ci',
    ...
    shell: {
      git_create_branch: {
        command: 'git checkout -b codeship_release_bump'
      },
      git_checkout_master: {
        command: 'git fetch && git checkout master'
      },
      git_merge_to_master: {
        command: 'git merge codeship_release_bump'
      },
      git_push_to_master: {
        command: 'git push origin master --no-verify'
      },
      git_delete_branch: {
        command: 'git branch -D codeship_release_bump'
      }
    },
    // Project settings
    ...
    watch: {
      ...
      sass: {
        files: [
          ...
          '<%= yeoman.client %>/styles/**/*.{scss,sass}'
        ],
        tasks: ['sass', 'postcss']
      },
      gruntfile: {
      ...
      livereload: {
        files: [
          ...
          '<%= yeoman.client %>/assets/scripts/{,*//*}*.{png,jpg,jpeg,gif,webp,svg}'
        ],
        options: {
          livereload: true
        }
    ...
    filerev: {
      dist: {
        src: [
          ...
          //'<%= yeoman.dist %>/<%= yeoman.client %>/assets/fonts/*' // NOTE SS 27/10/15 Commented this out, css doesn't refer to the versioned filename, rather the default
        ]
      }
    },
    ...
    ngconstant: {
      options: {
        name: 'digitalVillageApp.constants',
    ...
    buildcontrol: {
      ...
      herokuDev: {
        options: {
          remote: 'git@heroku.com:village-dev.git',
          branch: 'master',
          //tag: pkg.version
        }
      },
      herokuDemo: {
        options: {
          remote: 'git@heroku.com:village-demo.git',
            branch: 'master',
          //tag: pkg.version
        }
      }
  ...
  grunt.registerTask('integration-test-server', function() {
    return grunt.task.run([
      'env:all',
      'env:test',
  ...
  grunt.registerTask('unit-test-client', function() {
    return grunt.task.run([
      'env:all',
      'env:test',
      'karma:unit'
  ...
  grunt.registerTask('release', [
    ...
    //'test',
    'build',
    'buildcontrol:herokuDemo'
  ]);

  grunt.registerTask('prerelease', [
    'shell:git_create_branch',
    'bump:prerelease',
    'shell:git_checkout_master',
    'shell:git_merge_to_master',
    'shell:git_push_to_master',
    'shell:git_delete_branch',
    'build',
    'buildcontrol:herokuDev'
  ]);

  grunt.registerTask('releasedev', [
    //'shell:git_create_branch',
    //'bump:prerelease',
    //'shell:git_checkout_master',
    //'shell:git_merge_to_master',
